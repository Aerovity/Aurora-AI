/**
 * Qwen 0.5B via CactusCompute React Native SDK
 *
...
/**
 * Call Qwen 0.5B via CactusCompute native SDK with personality selection
 * @param userInput - The user's question or prompt
 * @returns Promise with the model's response
 */
export async function callQwen3(userInput: string): Promise<LocalModelResponse> {
  try {
    // Get pre-initialized model instance from ModelManager
    const lm = ModelManager.getQwenInstance();
